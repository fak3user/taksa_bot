...
func handleGroupMessage(update tgbotapi.Update, botInstance *tgbotapi.BotAPI) tgbotapi.MessageConfig {
    if update.Message.IsCommand() {
        return processGroupCommand(update)
    } else if isNewMember(update, botInstance) {
        return processNewGroupMember(update)
    }
...
func handlePrivateMessage(update tgbotapi.Update, botInstance *tgbotapi.BotAPI) tgbotapi.MessageConfig {
    if update.Message.IsCommand() {
        return processPrivateCommand(update, botInstance)
    }
    return tgbotapi.NewMessage(update.Message.From.ID, "test") // default response
}

func processGroupCommand(update tgbotapi.Update) tgbotapi.MessageConfig {
    var msg tgbotapi.MessageConfig
...
func processNewGroupMember(update tgbotapi.Update) tgbotapi.MessageConfig {
    var msg tgbotapi.MessageConfig
...
func processPrivateCommand(update tgbotapi.Update, botInstance *tgbotapi.BotAPI) tgbotapi.MessageConfig {
    var msg tgbotapi.MessageConfig
    switch update.Message.Command() {
    case "start":
